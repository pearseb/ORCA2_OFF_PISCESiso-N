...
CONTAINS
   ...
   SUBROUTINE p4z_lim( kt, knt )
      ...
      IF( lk_iomput .AND. knt == nrdttrc ) THEN        ! save output diagnostics
        ...
        IF( iom_use( "LNton"   ) )   CALL iom_put( "LNton"  , xnanono3(:,:,:) * tmask(:,:,:) )  ! oxidised nitrogen limitation term
        IF( iom_use( "LDton"   ) )   CALL iom_put( "LDton"  , xdiatno3(:,:,:) * tmask(:,:,:) )  ! oxidised nitrogen limitation term
        IF( iom_use( "LNnh4"   ) )   CALL iom_put( "LNnh4"  , xnanonh4(:,:,:) * tmask(:,:,:) )  ! reduced nitrogen limitation term
        IF( iom_use( "LDnh4"   ) )   CALL iom_put( "LDnh4"  , xdiatnh4(:,:,:) * tmask(:,:,:) )  ! reduced nitrogen limitation term
