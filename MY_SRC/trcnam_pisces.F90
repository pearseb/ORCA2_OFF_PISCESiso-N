...
CONTAINS
   ...
   SUBROUTINE trc_nam_pisces
      ...
      !!
      NAMELIST/nampismod/ln_p2z, ln_p4z, ln_p5z, ln_ligand, ln_sediment, ln_c13, ln_n15, ln_o18
      !!----------------------------------------------------------------------
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nampismod in configuration namelist', lwp )
      ...
      IF(lwp) THEN                  ! control print
         ...
         WRITE(numout,*) '      Flag to use oxygen isotopes          ln_o18      = ', ln_o18
      ENDIF
      !
      IF(lwp) THEN                         ! control print
         WRITE(numout,*)
         ...
         IF( ln_o18 )       WRITE(numout,*) '   ==>>>   Oxygen isotope module is used'
      ENDIF
